...
include_once $_SERVER['DOCUMENT_ROOT'] . '/vendor/autoload.php'; // adjust the path if needed
...
// 0 is the largest digit (order: 1..9 then 0)
function digitOrder($d) {
    return $d == 0 ? 10 : $d;
}

// Generate 3-digit number where each next digit is >= previous
// Examples: 123, 445, 667, 670 (0 is treated as largest)
function generateThreeDigitNumber() {
    while (true) {
        ...
        if (
            digitOrder($d1) <= digitOrder($d2) &&
            digitOrder($d2) <= digitOrder($d3)
        ) {
            return $d1 . $d2 . $d3;
        }
    }
...
// Get jodi from open & close like 234 + 560 => 91

function getJodi($open, $close) {
    $sumOpen  = array_sum(str_split($open));
    $sumClose = array_sum(str_split($close));
...
function isRedTextOn($jodi) {
    $chars = str_split($jodi);
    return (count($chars) == 2 && $chars[0] == $chars[1]) ? 'on' : 'off';
}
