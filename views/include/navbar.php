...
<header class="w-full flex bg-[#1a1a1a] items-center justify-between fixed top-0 left-0 z-[9999] py-2">
    ...
    <div class="flex items-center justify-center px-4 py-2 gap-3">
        <div onclick="OpenSidebar(this)" id="opensidebar" class="hidden max-md:flex"><img src="/public/icons/bars.png" class="h-8" alt=""></div>
        <div onclick="CloseSidebar(this)" id="closesidebar" class="hidden max-md:hidden"><img src="/public/icons/close.png" class="h-8" alt=""></div>
    ...
    ?>
        <div class="relative w-[45vw] max-md:hidden">
    ...
    <div class="flex items-center space-x-5 px-4 ">
        <button class="text-white hover:text-gray-700 max-md:hidden">
...
<script>
    ...
    function OpenSidebar(ele) {
        console.log('jbbbaaiud')
        let sidebar = document.getElementById('mobsidebar');
        console.log(sidebar);
        sidebar.classList.remove('max-md:hidden');
        closebtn.classList.remove('max-md:hidden')
        closebtn.classList.add('max-md:flex')

        ele.classList.add('max-md:hidden');
    ...
    function CloseSidebar(ele) {
        ...
        ele.classList.add('max-md:hidden');
        openbtn.classList.remove('max-md:hidden')
        openbtn.classList.add('max-md:flex')
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        // If search elements don't exist on this page, just exit safely
        if (!searchInput || !dropdownn) return;
        ...
        const groupedOptions = {
            "Main": [{
                    ...
                    link: "/dashboard",
                    icon: `<svg class='h-5 w-5 text-gray-500' xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                         <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                {
                    label: "Market",
                    desc: "Manage all markets",
                    link: "/market-list",
                    icon: `<img src="/public/icons/game-console.png" class="h-5">`
                },
                {
                    label: "Result",
                    desc: "Manage daily results",
                    link: "/result-list",
                    icon: `<img src="/public/icons/result.png" class="h-5">`
                }
            ],

            "Starline": [{
                    label: "Starline",
                    desc: "Manage starline markets",
                    link: "/starline-list",
                    icon: `<img src="/public/icons/star.png" class="h-5">`
                },
                {
                    label: "Starline Result",
                    desc: "View & update starline results",
                    link: "/starline-result-list",
                    icon: `<img src="/public/icons/star.png" class="h-5">`
                }
            ],
            ...
            "Games": [{
                    label: "Final Ank",
                    desc: "Manage final ank",
                    link: "/final-ank-list",
                    icon: `<img src="/public/icons/result.png" class="h-5">`
                },
                {
                    label: "Free Game",
                    desc: "Golden ank free game",
                    link: "/golden-ank",
                    icon: `<img src="/public/icons/result.png" class="h-5">`
                }
            ],
            ...
            "Users": [{
                label: "Create User",
                desc: "Add new website users",
                link: "/create-user",
                icon: `<img src="/public/icons/user.png" class="h-5">`
            }],

            "Front CMS": [{
                    label: "Home Banner",
                    desc: "Edit homepage banners",
                    link: "/admin/front-cms/home-banner",
                    icon: `<img src="/public/icons/arrow-right.png" class="h-5">`
                },
                {
                    label: "Kuber Image",
                    desc: "Manage Kuber image",
                    link: "/admin/front-cms/kuber",
                    icon: `<img src="/public/icons/arrow-right.png" class="h-5">`
                },
                {
                    label: "Navbar Heading",
                    desc: "Change navbar headings",
        ...
        function renderDropdown() {
            ...
            if (filtered.length === 0) {
                dropdownn.innerHTML =
                    `<div class='px-4 py-2 text-gray-400'>No results found</div>`;
                return;
            }
            ...
            filtered.forEach((item, i) => {
                if (item.group !== currentGroup) {
                    ...
                    header.className =
                        "px-4 pt-3 pb-1 text-xs font-semibold text-gray-400 uppercase";
                ...
                div.className =
                    `flex items-start gap-3 px-4 py-2 cursor-pointer rounded-md transition ${
        ...
        searchInput.addEventListener("keydown", e => {
            ...
            if (e.key === "ArrowDown") {
                ...
                if (filtered[activeIndex]) {
                    window.location.href = filtered[activeIndex].link;
                }
